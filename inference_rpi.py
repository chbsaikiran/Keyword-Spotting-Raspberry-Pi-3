"""
Keyword spotting inference for Raspberry Pi 3 — Combined (SQ+AWQ) TFLite model.
...
class KeywordSpotter:
    """Keyword spotter using the combined SmoothQuant+AWQ TFLite INT8 model."""
    ...
    def __init__(self, model_path: str, config_path: str):
        ...
        self.interpreter = tflite.Interpreter(
            ...
            num_threads=4,   # RPi3 quad-core Cortex-A53
        )
        ...
        print(f"[Combined] Loaded: {model_path}")
    ...
    def predict(self, wave: np.ndarray) -> tuple:
        ...
        inp     = log_mel[np.newaxis].astype(np.float32)    # [1, T, n_mels]

        self.interpreter.set_tensor(self._inp_idx, inp)
        self.interpreter.invoke()
        logits = self.interpreter.get_tensor(self._out_idx)[0]  # [n_classes]
...
def run_realtime(spotter: KeywordSpotter, threshold: float):
    ...
    print(f"\n[Combined] Listening… (Ctrl+C to stop, threshold={threshold:.0%})")
    print("-" * 60)
    try:
        while True:
...
def run_benchmark(spotter: KeywordSpotter, n_runs: int = 100):
    ...
    print(f"\n[Combined] Benchmark ({n_runs} runs):")
...
def main():
    p = argparse.ArgumentParser(description="Keyword spotting — Combined TFLite — RPi3")
